...
string telnum[30] = { "88005553535","88005243535","882125553535","81145553535","86433531243","88345253535","8801225835","430052213535","8321553535","88666666535","88005421535","880055654335","880043253535","85505553535","88000553535","88905553535","88666553535","88007753535","88005553755","77005553535","88028883535","88065553535","88235553535","88215553535","88005553575","88004353535","86605553535","87005553535","73005553535","64305553535" };
int coll_num = 0;
struct Human
{
	Human()
...
void hash_table::find_index(string teleph, const int size)
{
	int hash = hash_function(teleph, size);
	int index = hash;
	while (array[index].teleph != teleph && index < size)
	{
		index++;
	}
	if (index >= size)
	{
		index = 0;
		while (array[index].teleph != teleph && index < hash)
		{
			index++;
		}
		if (index >= hash)
		{
			cout << "Человека с номером телефона: " << teleph << " нет" << endl;
		}
		else
		{
			cout << "Человек с номером телефона: " << teleph << " найден с индексом: " << index << " ))" << endl;
		}
	}
	else
	{
...
{
	int index = hash_function(tmp.teleph, size);
	int hash = index;
...
Human creator()
{
	...
	tmp.teleph = get_random_id();
	return tmp;
}
string get_random_name()
{
	return(surname[get_random_num()] + " " + name[get_random_num()] + " " + patronymic[get_random_num()] + " " + telnum[get_random_num()]);
}
...
string get_random_tel()
{
	return telnum[get_random_num()];
}
void show(const Human tmp)
{
	cout << "ФИО:           " << tmp.full_name << endl;
	cout << "Дата рождения: " << tmp.birthday << endl;
	cout << "Номер паспорта:    " << tmp.id_number << endl;
	cout << "Номер телефона:    " << tmp.teleph << endl;
}
void print_Humans(const Human* const array, const int size)
{
