...
int listya_kol(Branch*& aBranch)

{
	if (NULL == aBranch) return 0; //Если дерева нет, выходим
	if (aBranch->LeftBranch==NULL&&aBranch->RightBranch==NULL)
	{
		listya++;
	}
	listya_kol(aBranch->LeftBranch); //Обошли левое поддерево
	listya_kol(aBranch->RightBranch); //Обошли правое поддерево
	return listya;
}
...

{
	...
	cout << "Проверка дерева на пустоту после заполнения: " << endl;
	is_Empty(Root);
	cout << endl;
	...
	print(Root);
	cout << endl;
	cout << "Количество листьев: " << listya_kol(Root) << endl;
	FreeTree(Root);
