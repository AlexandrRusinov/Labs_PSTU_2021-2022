...
int** help; // переменна оценивания пути
int* result; // результат - массив пути
...
int amountVerts; // количество вершин

template<class T>
class Graph
{
	vector<T> vertList; // массив вершин
	bool* visitedVerts = new bool[vertList.size()]; // массив, который запоминает пройденные вершины
public:
	...
	int GetVertPos(const T& vertex); // получение индекса вершины
	bool IsEmpty(); // проверка на пустоту
	bool IsFull(); // проверка на заполненность
	int GetAmountVerts(); // получение количества вершин
	int GetAmountEdges(); // получение количество ребер
	int GetWeight(const T& vertex1, const T& vertex2); // получение веса между вершинами
	void Print(); // вывод таблицы смежности
};


int R;
struct vertCoord // структура координат
{
	int x, y;
};
vertCoord vertC[20];
Graph<int> graph; // создание объекта класса Graph
...
void Graph<T>::InsertVertex(const T& vertex) {
	if (!this->IsFull()) {
		...
		cout << "Невозможно добавить новую вершину! Граф уже заполнен! " << endl;
		return;
	}
}
//
...
int Graph<T>::GetAmountEdges() {
	...
	if (!this->IsEmpty()) { // проверяем, что граф не пустой
...
//
// проверка графа на заполненность
//
template<class T>
bool Graph<T>::IsFull() {
