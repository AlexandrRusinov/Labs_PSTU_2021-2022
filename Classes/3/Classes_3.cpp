#include <iostream> // 12 вариант. Дружественная функция/класс имеет доступ к приватным полям класса, хотя не является членом класса(описана за границей). С помощью приватных полей делается "инкапсуляция"
