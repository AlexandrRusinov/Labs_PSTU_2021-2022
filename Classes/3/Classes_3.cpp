...
ostream& operator<<(ostream& out, const Pair& tmp) // перегрузка оператора вывода, всегда реализуются как внешние дружественные функции // перегрузка нужна для работы с несколькими полями за раз(независимо от типа полей), чтобы разделить их двоеточием
{
